...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-[94vh] overflow-hidden flex flex-col aurora-bg noise-overlay">
      ...
      {/* Blobs */}
      <div className="pointer-events-none absolute -bottom-40 -left-40 h-[800px] w-[800px] rounded-full"
      ...
      <div className="pointer-events-none absolute -right-20 -top-20 h-[700px] w-[700px] rounded-full"
        style={{ background: "radial-gradient(circle, rgba(99,102,241,0.45) 0%, rgba(99,102,241,0.15) 40%, transparent 70%)", animation: "blob-drift-alt 10s ease-in-out infinite", filter: "blur(40px)" }} />
      <div className="pointer-events-none absolute bottom-0 right-[20%] h-[500px] w-[500px] rounded-full"
        style={{ background: "radial-gradient(circle, rgba(168,85,247,0.3) 0%, rgba(168,85,247,0.08) 40%, transparent 70%)", animation: "blob-drift 18s ease-in-out infinite 4s", filter: "blur(50px)" }} />

      {/* Dot grid */}
      <div className="pointer-events-none absolute inset-0 grid-pulse"
        style={{ backgroundImage: "radial-gradient(circle, rgba(255,255,255,0.5) 1px, transparent 1px)", backgroundSize: "36px 36px" }} />

      {/* ── Cinematic illustration (right side, anchored bottom) ─ */}
      <div className="pointer-events-none absolute inset-0 z-[2] hidden items-end justify-end md:flex">
        <HeroIllustration className="h-[82%] w-[72%]" />
      </div>
      {/* Mobile: faint background illustration */}
      <div className="pointer-events-none absolute inset-0 z-[2] flex items-end justify-center opacity-20 md:hidden">
        <HeroIllustration className="h-[55%] w-full" />
      </div>
      ...
      <div
        ...
        style={{
          background: "linear-gradient(to right, #060d1e 30%, rgba(6,13,30,0.9) 46%, rgba(6,13,30,0.5) 64%, rgba(6,13,30,0.1) 80%, transparent 92%)",
        }}
      />
      {/* Bottom vignette for stats strip */}
      <div
        className="pointer-events-none absolute bottom-0 left-0 right-0 z-[3] h-40"
        style={{ background: "linear-gradient(to top, rgba(6,13,30,0.8) 0%, transparent 100%)" }}
      />
